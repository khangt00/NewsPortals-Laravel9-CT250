...
{{-- sidebar Advertisement 'top' --}}
    <div class="widget">
        @foreach($global_sidebar_top_data as $item)
        <div class="ad-sidebar" >
            @if($item->sidebar_ad_url == '')
            <img src="{{ asset('uploads/'.$item->sidebar_ad) }}" alt="" style="" ;>
            @else
            <a href="{{ $item->sidebar_ad_url }}"><img src="{{ asset('uploads/'.$item->sidebar_ad) }}" alt=""
                    style="width:100%" ;></a>
            @endif
        </div>
        @endforeach
{{-- end sidebar Advertisement 'top' --}}
    </div>
    ...
    <div class="widget">
        ...
        <div class="tag">
            @php
                $all_tags = \App\Models\Tag::select()->distinct()->get();
            @endphp
            ...
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill"
                        ...
                        aria-selected="true">{{RECENT_POPULAR}}</button>
                </li>
                <li class="nav-item" role="presentation">
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                    ...
                    @php
                         ...
                         ->where('language_id',$current_language_id)->orderBy('id','desc')->get();
                    @endphp
                ...
                <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    @php
                        ...
                        ->where('language_id',$current_language_id)->orderBy('visitors','desc')->get();
                    @endphp
    ...
    <div class="widget">
        ...
        <div class="poll">
            ...
            <div class="table-reponsive">
                <table class="table table-bordered">
                    <tr>
                        <td style="widh: 100%;">
                         {{ YES }} 
                         ({{ $online_poll_data?->yes_vote }})
                        </td>
                        <td>
                            <div class="progress">
                                <div class="grogress-bar bg-success" role="progressbar" 
                                style="width: {{ $total_yes_percentage }}%" 
                                aria-valuenow="{{ $total_yes_percentage }}"
                                aria-valuemin="0" aria-valuemax="100%">{{ $total_yes_percentage }}%</div>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            {{ NO }} 
                            ({{ $online_poll_data?->no_vote }})
                        </td>
                        <td>
                            <div class="progress">
                                <div class="grogress-bar bg-warning" role="progressbar" 
                                style="width:{{ $total_no_percentage }}%" 
                                aria-valuenow="{{ $total_no_percentage }}"
                                aria-valuemin="0" aria-valuemax="100%">{{$total_no_percentage }}%</div>
                            </div>
                        </td>
                    </tr>
            ...
            <a href="{{ route('poll_previous') }}" class="btn btn-primary old" style="margin-top: 0;">{{ OLD_RESULT }}</a>
        </div>
        @endif
        ...
        <div class="answer-option">
            <form action="{{ route('poll_submit') }}" method="post">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="vote"  value="Yes" checked>
                    <label class="form-check-label" for="poll_id_2">{{ YES }}</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="vote"  value="No">
                    <label class="form-check-label" for="poll_id_2">{{ NO }}</label>
                </div>
                <div class="form-control">
                    <button type="submit" class="btn btn-warning">{{ SUBMIT }}</button>
                    <a href="{{ route('poll_previous') }}" class="btn btn-primary old">{{ OLD_RESULT }}</a>
                </div>
            </form>
        </div>
        @endif
...
{{-- Sidebar Advertisement 'Bottom' --}}
    <div class="widget">
        ...
        <div class="ad-sidebar">
            @if($item->sidebar_ad_url == '')
            <img src="{{ asset('uploads/'.$item->sidebar_ad) }}" alt="" style="width:100%" ;>
            @else
            <a href="{{ $item->sidebar_ad_url }}"><img src="{{ asset('uploads/'.$item->sidebar_ad) }}" alt=""
                    style="width:100%" ;></a>
            @endif
...
{{-- End Sidebar Advertisement 'Bottom' --}}
